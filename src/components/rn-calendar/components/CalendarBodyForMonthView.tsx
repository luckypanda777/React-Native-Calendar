...
import { getWeeksWithAdjacentMonths } from '../utils'
...
function _CalendarBodyForMonthView<T extends ICalendarEventBase>({
  ...
  const onCellLayout = ({ nativeEvent }, date, rowIndex, cellIndex) => {
    rowRef.current[rowIndex]?.measure((x, y, width, height, pageX, pageY) => {
      let posY = pageY;
      cellRef.current[cellIndex]?.measure((x, y, width, height, pageX, pageY) => {
          ...
          {week
            ...
            .map((date, ii) => (
              <TouchableOpacity
                ...
                key={ii}
                ref={el => cellRef.current[ii] = el}
                onLayout={e => onCellLayout(e, date, i, ii)}
              >
                <TouchableOpacity
                  onPress={() =>
                    date &&
