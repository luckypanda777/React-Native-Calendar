...
export default class DayView extends React.PureComponent {
  ...
  _renderRedLine() {
    ...
    const difference = moment().startOf('day') - moment(this.props.currentDate, 'DD MMMM YYYY').startOf('day');
    // const difference = 0
    return (
      difference === 0 ? 
        <View
    ...
    return range(1, 7).map((i, index) => {
      ...
      return [
        ...
        i === start ? null : (
          <View
            key={`line${i}`}
            style={[styles.line, { top: offset * index, width: width - 61 }]}
          />
        ),
        <View
          key={`lineHalf${i}`}
          style={[
            styles.line,
            { top: offset * (index + 0.5), width: width - 61 },
          ]}
        />,
      ];
    });
